            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        ...
                        <form method="POST" action="{{ route('barang.prosesor.store',['barang' => $barang['id']]) }}" >
                            @csrf

                            <div class="form-group">
                                <label for="tipeRam_id">tipe Prosesor</label>
                                <select name="tipeProsesor_id" id="" class="form-control">
                                    @foreach ($tipeProsesor as $item)
                                        <option value="{{ $item['id'] }}">{{ $item['namaTipeProsesor'] }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="kapasitas">kapasitas prosesor</label>
                                <input type="text" class="form-control" name="kapasitas">
                            </div>

                            <div class="form-group">
                                <label for="name">keterangan prosesor</label>
                                <textarea name="keteangan" id="" cols="30" rows="10" class="form-control"></textarea>
                            </div>
